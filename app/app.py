...
with open("data/grid.json", "r") as fin:
    ...
    parser.add_argument('xy', required=True, type=str, help='Coordinate string missing or unable to convert to decimal value. Must be formatted as comma-separated string in format x,y,x,y...')
    parser.add_argument('mode', type=str, choices=["origin", "destination"], help='Mode string must be one of (origin, destination)')
            ...
            for trip_grid_id in feature["properties"][key].keys():
                for grid_id in source_ids:
                    ...
                    if trip_grid_id == grid_id:
                        ...
                        feature["properties"]["current_count"] += feature["properties"][key][trip_grid_id]["count"]
